...
parser.add_argument("--config", default='./config/deeplabv1_voc12.yaml', type=str, help="config")
...
def train(config=None):
    ...
    for idx, item in enumerate(pretrained_dict.items()):
        ...
        new_state_dict[new_keys[idx]] = item[1]
    ...
    for epoch in range(max_epoch):
        ...
        for _, data in tqdm(enumerate(train_loader), total=len(train_loader), ascii=' 123456789>', dynamic_ncols=True):
            ...
            # resize labels
            resized_labels = resize_labels(labels, size=outputs.shape[2:])
            resized_labels = resized_labels.to(device)
            #resized_labels = F.interpolate(input=labels.unsqueeze(1), size=[41, 41], mode='nearest')
            #resized_labels = resized_labels.long()[:,0,:,:]

            # zero the parameter gradients
            optimizer.zero_grad()
    
            # forward + backward + optimize
            loss = criterion(outputs, resized_labels)
